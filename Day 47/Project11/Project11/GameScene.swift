...
//  Project11
//
//  Created by Martin Kabát on 12.10.2022.
//
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    override func didMove(to view: SKView) {
        ...
        makeBouncer(at: CGPoint(x: 748, y: 0))
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if object.contains(editLabel) {
            ...
            if editingMode {
                ...
                ball.position = CGPoint(x: location.x, y:(scene?.size.height)!)
                ball.name = "ball"
                
                guard let children = scene?.children else { return }
                
                var ballChildrenCount = 0
